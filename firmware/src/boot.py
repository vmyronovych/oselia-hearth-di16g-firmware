"""OTA loader -- runs at MicroPython startup, BEFORE the app.

Installed ONCE via USB (provisioning); it is NEVER part of an OTA bundle, so no
update can brick the boot path. Deliberately self-contained (no app imports) and tiny.
...
Responsibilities (OTA_SPEC.md "Boot-confirm / auto-revert"):
  ...
  3. import + run the app's main() from that slot.
