...
> <a name="migrate-0-9-0"></a>**⚠️ One-time exception — moving to 0.9.1 (USB) · Перехід на 0.9.1 (одноразово, через USB)**
>
> 🇬🇧 **0.9.1 is the only release that must be applied over USB, not from Home Assistant.**
...
> (`oselia provision`). If you click *Install* in HA for 0.9.1 it safely rolls back and stays
> on the old version. **From 0.9.1 onward, normal one-click HA updates work again.** Newly
> shipped units already have 0.9.1 and need nothing.
>
> 🇺🇦 **0.9.1 — єдиний реліз, який треба встановити через USB, а не з Home Assistant.**
...
> (`oselia provision`). Якщо натиснути *Встановити* в HA для 0.9.1, оновлення безпечно
> відкотиться і залишиться стара версія. **Починаючи з 0.9.1, звичайні оновлення в один клік
> через HA знову працюють.** Нові пристрої вже йдуть з 0.9.1 — робити нічого не треба.
...
застосовується відновлення через USB (див. `flashing.md`).
...
last resort it can be recovered over USB (see `flashing.md`).
