...
unreliable on a running unit — its USB CDC isn't dependably available while the dual-core
firmware runs, and a cold boot can wedge USB enumeration (see the firmware boot-wedge notes).
(As of fw 0.7.0 the **hardware watchdog lives on core 1**, not core 0 — an MCP/I²C stall on
core 0 never resets the board; the WDT guards the network core.) So the wizard first tries a
